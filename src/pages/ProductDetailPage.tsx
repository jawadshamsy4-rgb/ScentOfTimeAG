...
import DiscoveryPerfumeSelector from "@/components/DiscoveryPerfumeSelector";
...
const ProductDetailPage = () => {
  ...
  const categoryDiscountedPrice = getDiscountedPrice(currentPrice, product.category, discounts);
  const categoryDiscountLabel = getDiscountLabel(product.category, discounts);

  // Discovery set logic
  const isDiscovery = product.category.toLowerCase() === "discovery";
  const discoverySource = product.discoverySourceCategory
    || (product.name.toLowerCase().includes("female") ? "Women" : "Men");
  const discoveryCount = (() => {
    if (!isDiscovery || !currentVariant) return 0;
    const src = `${currentVariant.bestFor || ""} ${currentVariant.size || ""}`.toLowerCase();
    const match = src.match(/(\d+)\s*pieces?/);
  ...
  const validateDiscovery = (): boolean => {
    ...
    if (filled < discoveryCount) {
      toast({ title: "Please select perfumes for all slots.", variant: "destructive" });
      return false;
    }
    return true;
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-24 pb-4 max-w-[1440px] mx-auto px-6 lg:px-10">
        <Link to="/" className="inline-flex items-center gap-2 font-body text-[12px] font-medium tracking-wide text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft size={14} /> Back to All Perfumes
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          <div className="lg:sticky lg:top-28 animate-fade-in" style={{ animationDelay: "0.15s" }}>
            ...
            {product.variants.length > 0 && (
              <div className="mb-8">
                <p className="font-body text-[11px] font-semibold tracking-[0.2em] uppercase text-muted-foreground mb-3">Select Size</p>
            ...
            {/* Discovery perfume selector */}
            {isDiscovery && discoveryCount > 0 && (
              <DiscoveryPerfumeSelector
                count={discoveryCount}
                sourceCategory={discoverySource}
                perfumeSize={currentVariant?.perfumeSize || "6ML"}
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <Button variant="cta" size="xl" className="flex-1 gap-2 h-[48px] sm:h-14 py-3 sm:py-0 px-5 sm:px-10 items-center justify-center" disabled={!product.inStock} onClick={() => {
                ...
                {product.inStock ? <>Add to Cart — <span className="font-bold">৳{currentPrice}</span></> : 'Out of Stock'}
              </Button>
            ...
            <div className="flex flex-wrap gap-4 pt-6 border-t border-border">
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="font-body text-[11px]">6+ Hours Longevity</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {product.story && (
        <section className="py-16 lg:py-24">
          <div className="max-w-3xl mx-auto px-6 lg:px-10 text-center">
            ...
            <h2 className="font-display text-3xl md:text-4xl font-light text-foreground mb-8">
              Behind the <span className="italic font-semibold text-gold">Scent</span>
            </h2>
          ...
          <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
            <div className="text-center mb-12">
              ...
              <h2 className="font-display text-3xl md:text-4xl font-light text-foreground">
                Related <span className="italic font-semibold text-gold">Fragrances</span>
              </h2>
            </div>
      ...
      {product.variants.length > 0 && (
        <OrderModal
          ...
          selectedPerfumes={isDiscovery ? discoverySelections.filter(Boolean) : undefined}
        />
      )}
    </div>
  );
