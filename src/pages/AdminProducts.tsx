...
interface ProductForm {
  ...
  story: string;
  in_stock: boolean;
  discovery_source_category: string;
...
const AdminProducts = () => {
  ...
  const openEdit = (p: DbProduct) => {
    ...
    setForm({
      ...
      discovery_source_category: ((p as any).discovery_source_category as string) || "",
      variants,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log("[AdminProducts] Submit clicked", { editingId, form });
    if (!form.name.trim()) {
    ...
    try {
      ...
      const mainPrice = form.variants.length > 0
        ? Math.max(...form.variants.map((v) => Number(v.price) || 0))
        : form.price;
      ...
      const payload = {
        ...
        discovery_source_category: form.category.toLowerCase() === "discovery" ? (form.discovery_source_category || null) : null,
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <Button asChild variant="outline" size="sm" className="h-7 px-2 text-[11px]">
                <Link to="/"><Home size={12} className="mr-1" /> Home</Link>
              </Button>
            </div>
            <p className="font-body text-[10px] md:text-[12px] text-muted-foreground">Scent of Time — Admin Dashboard</p>
          </div>
        ...
        <DialogContent className="sm:max-w-2xl bg-card border-border max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div>
              ...
              {(form.product_images.length + pendingImageFiles.length) < 5 && (
                <label className="inline-flex items-center gap-2 cursor-pointer bg-secondary hover:bg-muted px-4 py-2.5 rounded-lg font-body text-[12px] text-foreground transition-colors">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const remaining = 5 - form.product_images.length - pendingImageFiles.length;
            ...
            {form.category.toLowerCase() === "discovery" && (
              <div>
                <label className="font-body text-[11px] font-semibold tracking-[0.15em] uppercase text-muted-foreground mb-1.5 block">
                  Source Category for Perfume Selection
                </label>
                <select
                  value={form.discovery_source_category}
                  onChange={(e) => setForm((p) => ({ ...p, discovery_source_category: e.target.value }))}
                  className="w-full bg-background border border-border rounded-lg px-4 py-2.5 font-body text-[13px] text-foreground focus:outline-none focus:ring-2 focus:ring-gold/30"
                >
                ...
                <p className="font-body text-[11px] text-muted-foreground mt-1">
                  Only products from this category will appear in the customer's "Select Your Perfumes" dropdowns.
                </p>
              </div>
            )}
            ...
            <div>
              ...
              {form.category.toLowerCase() === "discovery" && (
                <p className="font-body text-[11px] text-muted-foreground mb-2">
                  For Discovery sets, set the number of perfumes the customer can select for each option.
                </p>
              )}
              <div className="space-y-2">
                {form.variants.map((v, i) => {
                  const isDiscovery = form.category.toLowerCase() === "discovery";
                  const piecesMatch = (v.bestFor || "").match(/(\d+)\s*pieces?/i);
                  const piecesValue = piecesMatch ? piecesMatch[1] : "";
                  return (
                      ...
                      <div className="flex-1 flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            updateVariant(i, "bestFor", n ? `${n} Pieces` : "");
                          }}
                          placeholder="Number of perfumes to select"
                        ...
                        />
                        <span className="font-body text-[11px] text-muted-foreground whitespace-nowrap">Pieces</span>
                        <input
                          type="text"
                          value={v.perfumeSize || ""}
                          onChange={(e) => updateVariant(i, "perfumeSize", e.target.value)}
                          placeholder="Each (e.g. 6ML)"
                          className="w-28 bg-background border border-border rounded-lg px-3 py-2 font-body text-[12px] text-foreground focus:outline-none focus:ring-2 focus:ring-gold/30"
                        />
                      </div>
                    ) : (
                    <input
